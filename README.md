...
- lint で出る `UNSAFE_VAR_ASSIGNMENT` 警告 2 件は vendor 内(DOMPurify / highlight.js の
  公式ビルド内部)のもので、既知ライブラリとして審査上問題ない(エラーではないので提出可能)。
  拡張機能自身のコードは innerHTML を使わず、DOMPurify の `RETURN_DOM_FRAGMENT` で
  サニタイズ済み DOM を直接挿入している
